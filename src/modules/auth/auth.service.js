...
export async function registerUser(userData){
  ...
  const contraseniaHasheada=await bcrypt.hash(userData.password,valorAleatorio);
...
export async function loginUser(correo,password) {
  const usuarioExistente=await Usuario.findOne({correo}).select('+password').populate('id_rol');
  if (!usuarioExistente){
